 ...
 *
 * Runs preview/local auth checks, then restarts with VERCEL_ENV=production
 * to prove the hard-404 payload contains no dashboard/login serialization.
 */
...
const DASHBOARD_LEAK_PHRASES = [
  ...
  "consultationFunnel",
  "brandDemand",
  "executive-dashboard-username",
  "executive-dashboard-password",
  "hgd_ed_session",
];
...
function assertNoDashboardLeak(body, label) {
  for (const phrase of DASHBOARD_LEAK_PHRASES) {
    assert(
      !body.includes(phrase),
      `${label} leaked dashboard phrase: ${phrase}`,
    );
  }
  assert(
    !/<title>[^<]*Executive[^<]*<\/title>/i.test(body),
    `${label} leaked Executive title`,
  );
  assert(!body.includes(PASSWORD), `${label} password in body`);
}
...
async function runPreviewChecks(secrets, results) {
  {
...

  {
    ...
    assert(body.includes("Executive access"), "missing login headline");
  ...
  {
    // Missing auth env still fails closed outside production.
    results.push("11 preview missing-env covered by unit tests: pass");
  }
}

async function runProductionHard404Checks(secrets, results) {
  const paths = [
    "/executive-dashboard",
    "/executive-dashboard/login",
    "/executive-dashboard/test",
    "/executive-dashboard/nested/smoke",
  ];

  for (const path of paths) {
    const res = await fetch(`${BASE}${path}`, { redirect: "manual" });
    assert(res.status === 404, `prod HTML ${path} status ${res.status}`);
    assert(!res.headers.get("location"), `prod HTML ${path} unexpected redirect`);
    const body = await res.text();
    assertNoDashboardLeak(body, `prod HTML ${path}`);
    assert(
      !body.includes(secrets.hash) && !body.includes(secrets.secret),
      `prod HTML ${path} auth secret leak`,
    );
  }
  results.push("12 production HTML hard-404 (root/login/nested): pass");
  ...
  for (const path of paths) {
    ...
    assert(res.status === 404, `prod RSC ${path} status ${res.status}`);
    const body = await res.text();
    assertNoDashboardLeak(body, `prod RSC ${path}`);
  }
  results.push("13 production RSC/Flight hard-404: pass");

  {
    const res = await fetch(`${BASE}/`);
    assert(res.status === 200, `prod public home ${res.status}`);
    const body = await res.text();
    assertNoDashboardLeak(body, "prod public home");
    results.push("14 production public home unaffected: pass");
  }

  {
    // Even with a valid-looking session cookie, production stays hidden.
    const token = createSessionToken(USERNAME, secrets.secret);
    const res = await fetch(`${BASE}/executive-dashboard`, {
      redirect: "manual",
      headers: { cookie: `hgd_ed_session=${token}` },
    });
    assert(res.status === 404, `prod authed cookie status ${res.status}`);
    assertNoDashboardLeak(await res.text(), "prod authed cookie");
    results.push("15 production ignores session cookie: pass");
  }
}
...
async function run() {
  ...
  // Preview/local: ensure we are not accidentally in production mode.
  delete baseEnv.VERCEL_ENV;

  const results = [];
  let server = startServer(baseEnv);

  try {
    await waitForServer();
    await runPreviewChecks(secrets, results);
    ...
    await waitForServer();
    await runProductionHard404Checks(secrets, results);
      ...
      JSON.stringify(
        {
          ...
          results,
          serverLogTail: server.getLog().slice(-800),
        },
        null,
        2,
      ),
