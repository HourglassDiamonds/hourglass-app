...
function mutation(
  ...
  return {
    ...
    priorHourglassQuoteCents: input.quote.calculation.hourglassQuoteCents,
    newHourglassQuoteCents: input.next.calculation.hourglassQuoteCents,
...
export async function overrideRepairQuote(
  ...
  return mutate(deps, input, (quote, now, actor) => {
    ...
    const recalculated = calculateRepairQuote({
      ...
      gold: {
        usdCentsPerTroyOz: quote.goldUsdCentsPerTroyOz,
        asOfDate: quote.goldAsOfDate,
        source: quote.goldInputSource,
        baselineUsdCentsPerTroyOz: quote.goldBaselineUsdCentsPerTroyOz,
      },
      markupRatioPermyriad: quote.markupRatioPermyriad,
      lines: quote.lines.map((line) => ({
        ...
        manualMetalDeltaCents: line.goldSensitive ? null : line.metalDeltaCents,
      })),
      quoteDate: quote.goldAsOfDate,
