...
/** Use the same @napi-rs/canvas build pdfjs-dist resolves (not root 1.0.0). */
...
function getPdfJsNodeCanvasModule(): PdfJsNodeCanvas {
...
function installPdfJsCanvasPolyfills(canvasPkg: PdfJsNodeCanvas): void {
...
function createPdfJsNodeCanvasFactory(canvasPkg: PdfJsNodeCanvas) {
  return {
...
async function renderSarinePdfPageAtScale(
  pdfBytes: Buffer,
  pageNumber: number,
  scale: number,
  canvasPkg: PdfJsNodeCanvas,
  ...
  try {
    ...
    logCalibrationRuntimeCheck({
      operation: "sarine-pdf-render",
      parserPath: "gcal-sarine-4cs",
      renderScale: scale,
...
export async function ocrGcalSarineProportionRegionWithDiagnostics(
  ...
  for (const scale of [GCAL_SARINE_PAGE_SCALE, 3, 2]) {
    const attempt = await renderSarinePdfPageAtScale(
      pdfBytes,
      1,
      scale,
      canvasPkg,
  ...
  diagnostics.ocrError = ocr.error;
