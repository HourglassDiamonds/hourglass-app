...
export function AddClientForm({ submissionId }: { submissionId: string }) {
  const givenId = useId();
  ...
  const [state, formAction, pending] = useActionState(
    saveManualClient,
    null as SaveManualClientState | null,
  );
        ...
        <div className="mt-6">
          ...
          {state.conflictingPersonIds && state.conflictingPersonIds.length > 0 ? (
            <ul className="mt-3 space-y-2">
              {state.conflictingPersonIds.map((personId) => (
                <li key={personId}>
                  <Link
