import type { ReactNode } from "react";
import { ConciergeOperatingNav } from "./concierge-operating-nav";

export type ConciergeShellVariant = "home" | "document" | "book";

export function ConciergeShell({
  children,
  footer,
  variant = "document",
}: {
  children: ReactNode;
  footer?: ReactNode;
  variant?: ConciergeShellVariant;
}) {
  const frame =
    variant === "home" || variant === "book"
      ? "relative mx-auto w-full max-w-[34rem] px-5 pb-[calc(5.75rem+env(safe-area-inset-bottom))] pt-6 md:max-w-[75rem] md:px-8 md:pb-16 md:pt-8"
      : "relative mx-auto w-full max-w-[34rem] px-5 pb-[calc(5.75rem+env(safe-area-inset-bottom))] pt-6 md:max-w-[38rem] md:px-8 md:pb-16 md:pt-10";

  return (
    <div
      data-concierge-shell
      data-concierge-variant={variant}
      data-operating-shell
      className="relative min-h-[100dvh] overflow-x-hidden bg-[#14110f] text-[#efe8de]"
    >
      <div
        aria-hidden
        className="pointer-events-none absolute inset-x-0 top-0 h-[28rem] bg-[radial-gradient(ellipse_at_top,_rgba(173,145,100,0.08),_transparent_58%)]"
      />
      <ConciergeOperatingNav />
      <div className={frame}>
        {children}
        {footer}
      </div>
    </div>
  );
}
