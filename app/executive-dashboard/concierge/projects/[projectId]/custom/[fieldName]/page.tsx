...
export default async function ConciergeCorrectCustomDetailPage({
  params,
}: {
  params: Promise<{ projectId: string; fieldName: string }>;
}) {
  const { projectId, fieldName } = await params;
  if (!isProjectIdParam(projectId) || !isCustomOperatingDetailField(fieldName)) {
    return (
      <ConciergeShell>
  ...
  if (desk.desk.operatingLayer.kind !== "custom_new_jewelry") {
    return (
      <ConciergeShell>
        <Link
